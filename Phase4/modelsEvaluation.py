...
def plot_learning_curve(estimator, title, X, y, axes=None, ylim=None, cv=None,
    ...
    if not loading_learning_curves:
        train_sizes, train_scores, test_scores, = \
            learning_curve(estimator, X, y, cv=cv, n_jobs=n_jobs,
                           train_sizes=train_sizes, scoring = scoring, verbose = 3)
        
        dump([train_sizes,train_scores, test_scores], learning_folder + "learning_curve_data_" + modelName + ".joblib")
    else:
        train_sizes, train_scores, test_scores = load(learning_folder + "learning_curve_data_" + modelName + ".joblib");
...
loading_learning_curves = True; #Load the pre-calculated learning curves;
loading_metrics = True; #Load the precalculated metrics;
do_learning_curves = True; #draw learning curves.
datasetName = "datasets/Aggregate-dataset.arff" #dataset name. Default: "datasets/Aggregate-dataset.arff"
...
learning_folder = folder + "learning_curves/"
...
print(len(X))
metrics_file = open(folder + 'CONV_metrics.csv', mode='w');
...
for selection1 in selected_models:
    ...
    dump(clf, model_folder + "model_" + modelName + ".joblib");
